...
/* format_ext4: Creates an ext4 file system.
 * Returns 0 on success, -1 on error
 */
extern struct fs_info info;
int format_ext4(char *fs_type, char *fs_real_blkdev, char *fs_mnt_point, long int fs_blksize)
{
    int status;
    off64_t off;
    int fd;
...
int main(int argc, char* argv[])
{
    ...
    if (is_valid_fs(fs_type, fs_real_blkdev, fs_mnt_point)) {
        ...
        status = format_ext4(fs_type, fs_real_blkdev, fs_mnt_point, fs_blksize);
    }
    return status;
}
